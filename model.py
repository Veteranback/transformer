...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, n_heads, d_queries, d_values, dropout, in_decoder=False,device='cpu'):
        """
        ...
        self.device=device
    def forward(self, query_sequences, key_value_sequences, key_value_sequence_lengths,device='cpu'):
        """
        Forward prop.
        ...
        # MASK 1: keys that are pads 将填充字符对应的权重进行掩码
        ...
        #x.unsqueeze(0)相当于是[x] 即给x增加一个维度，expand_as则是将维度扩展为attention_weights
        #not_pad_in_keys 这里维度和attention_weights
        ...
        #后面一部分是将每一个序列的长度矩阵 扩展为attention_weights维度，
        # 然后比较当前维度和序列维度，如果长度超过序列维度，那么序列设置为False 否则为True
...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab_size,tgt_vocab_size, positional_encoding, d_model=512, n_heads=8, d_queries=64, d_values=64,
                 d_inner=2048, n_layers=6, dropout=0.1,args=None):
        """
        :param vocab_size: size of the (shared) vocabulary
        :param positional_encoding: positional encodings up to the maximum possible pad-length
